...
export const reportesEstadoEnum = pgEnum("reportes_estado_enum", [
  "PENDIENTE",
  "ACTIVO",
  "DEVUELTO",
]);
...
export const usuarios = pgTable(
  ...
  (table) => [
    index("idx_usuarios_correo").using(
      "btree",
      table.correo.asc().nullsLast().op("text_ops"),
    ),
    index("idx_usuarios_rol_id").using(
      "btree",
      table.rolId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.rolId],
...
export const reportes = pgTable(
  "reportes",
  {
    ...
    ubicacionEncontrado: varchar("ubicacion_encontrado", {
      length: 255,
    }).notNull(),
  ...
  (table) => [
    index("idx_reportes_categoria_id").using(
      "btree",
      table.categoriaId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_reportes_codigo").using(
      "btree",
      table.codigo.asc().nullsLast().op("text_ops"),
    ),
    index("idx_reportes_estado").using(
      "btree",
      table.estado.asc().nullsLast().op("enum_ops"),
    ),
    index("idx_reportes_feed").using(
      "btree",
      table.estado.asc().nullsLast().op("timestamp_ops"),
      table.createdAt.asc().nullsLast().op("timestamp_ops"),
    ),
    index("idx_reportes_usuario_reportante_id").using(
      "btree",
      table.usuarioReportanteId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const identidadesUsuario = pgTable(
  "identidades_usuario",
  {
    ...
    identificadorExterno: varchar("identificador_externo", {
      length: 255,
    }).notNull(),
    createdAt: timestamp("created_at", { mode: "string" })
      .default(sql`CURRENT_TIMESTAMP`)
  ...
  (table) => [
    index("idx_identidades_usuario_id").using(
      "btree",
    ...
    unique("uq_identidad_proveedor_externo").on(
      table.proveedor,
...
export const categorias = pgTable(
  ...
  (table) => [unique("categorias_nombre_key").on(table.nombre)],
);
...
export const roles = pgTable(
  ...
  (table) => [unique("roles_codigo_key").on(table.codigo)],
);
...
export const registrosAuditoria = pgTable(
  ...
  (table) => [
    index("idx_auditoria_creado_at").using(
      "btree",
      table.createdAt.asc().nullsLast().op("timestamp_ops"),
    ),
    index("idx_auditoria_entidad").using(
      "btree",
      table.entidadId.asc().nullsLast().op("text_ops"),
      table.tipoEntidad.asc().nullsLast().op("text_ops"),
    ),
    index("idx_auditoria_usuario_id").using(
      "btree",
      table.usuarioId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const devoluciones = pgTable(
  "devoluciones",
  {
    ...
    metodoVerificacion: varchar("metodo_verificacion", {
      length: 100,
    }).notNull(),
    observaciones: text(),
  ...
  (table) => [
    index("idx_devoluciones_reporte_id").using(
      "btree",
      table.reporteId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_devoluciones_usuario_receptor_id").using(
      "btree",
      table.usuarioReceptorId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_devoluciones_usuario_seguridad_id").using(
      "btree",
      table.usuarioSeguridadId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
